    ...
    namespace CrashHandler
    {
        ...
        static LONG WINAPI CustomCrashHandler(EXCEPTION_POINTERS *exceptionInfo)
        {
            ...
            for(EXCEPTION_RECORD * record = exceptionInfo->ExceptionRecord; record != nullptr; record = record->ExceptionRecord)
            {
                ...
                std::cout << ExceptionCodeToString(record->ExceptionCode)  << " occurred at address (PC): " << record->ExceptionAddress << std::endl;
                if(record->ExceptionCode == EXCEPTION_ACCESS_VIOLATION)
                {
                    for(int i = 0; i < depth; i++) std::cout << "\t";
                    switch(record->ExceptionInformation[0])
                    {
                        case 0: std::cout << "Failed reading at address " << record->ExceptionInformation[1]; break;
                        case 1: std::cout << "Failed writing at address " << record->ExceptionInformation[1]; break;
                        case 8: std::cout << "Data was tried to be interpreted as code at address " << record->ExceptionInformation[1] << ". How did you even do this!?!?!?"; break;
                    }
                    std::cout << std::endl;
                }
        ...
        static const char* ExceptionCodeToString(DWORD code)
        {
            switch (code)
            {
                ...
                default: return "UNKNOWN EXCEPTION (WHAT IS GOING ON HERE!?!?! ;>(";
            }
        }
