...
void HubertusSystem::Update(Engine::Entity entity, float deltaTime)
{
    ...
    {
        sceneManager->GetActiveScene<Game>().CreateBulletEnemy(entity, transform.GetGlobalTranslation() + glm::vec3{0, 0, 0.5f}, glm::vec3(hubertus.movement.direction, 0), hubertus.bulletSpeed);
        std::uniform_real_distribution<> distr(shootIntervalRange.first, shootIntervalRange.second);
        hubertus.shootTimer = distr(gen);
    }
...
void HubertusSystem::HandleDamage(Engine::Entity entity, Engine::Entity other)
{
    ...
    if(health.health <= 0)
    {
        Systems::enemyBehaviourSystem->Kill(entity, EnemyBehaviour::scores[EnemyBehaviour::Hubertus]);
    }
}
